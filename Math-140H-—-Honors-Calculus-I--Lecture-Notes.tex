...
\pandocbounded{\includegraphics[keepaspectratio]{06_activity_properties_of_functions_files/figure-pdf/fig-blank-axes-output-1.pdf}}
...
\item
  ...
  we already know.
\item
  A \textbf{bounded monotone sequence converges} (2), a fact equivalent
  to the Nested Interval Axiom; the paired form
...
\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week03_convergence_files/figure-pdf/cell-4-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week03_convergence_files/figure-pdf/cell-5-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week03_convergence_files/figure-pdf/cell-7-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week03_convergence_files/figure-pdf/cell-8-output-2.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week03_convergence_files/figure-pdf/cell-11-output-1.png}}
...
\chapter{Riemann Sums}\label{riemann-sums}

\section{Area of the Circle}\label{area-of-the-circle}

We start with a problem from antiquity: what is the area of a circle of
radius \(1\)? The classical idea (going back to Archimedes) is to
approximate the circle by figures whose area we \emph{can} compute ---
regular \(n\)-gons inscribed in the circle.

Split the \(n\)-th polygon (\(n\ge3\)) into \(n\) congruent triangles,
each with its apex at the center of the circle, two sides of length
\(1\), and apex angle \(\tfrac{2\pi}{n}\). Taking one of the two unit
sides as base, the height of the triangle is
\(h = \sin\!\big(\tfrac{2\pi}{n}\big)\), so \[
...
\] As \(n\) grows, the polygons hug the circle more and more tightly,
and \(A_n\) should approximate the area of the circle better and better.
...
\tightlist
\item
  \((A_n)\) is \textbf{increasing}.
\item
  \((A_n)\) is \textbf{bounded}: every inscribed \(n\)-gon is contained
  in the square of side length \(2\) circumscribing the circle, so
  \(A_n\le 4\) for all \(n\).
\end{itemize}

By the Monotone Convergence Theorem (2), \((A_n)\) converges. Its limit
is \(\pi\).

\begin{refremark}
Two points are glossed over here. Monotonicity is not obvious from the
picture, since the \(n\)-gon is not contained in the \((n+1)\)-gon; it
follows from the fact that \(\sin(x)/x\) is decreasing on \((0,\pi]\),
writing \(A_n = \pi\cdot\dfrac{\sin(2\pi/n)}{2\pi/n}\). The same
rewriting shows that identifying the limit as \(\pi\) amounts to knowing
\(\lim_{x\to0}\frac{\sin x}{x} = 1\) --- a limit we will return to when
we study derivatives of the trigonometric functions.

\label{rem-circle-area-details}

\end{refremark}

How can we generalize this idea to other areas? Triangles could work,
but it is not clear how to set this up \emph{methodically} for an
arbitrary region. For regions under the graph of a function, there is a
better choice: \textbf{rectangles}, whose area is as easy to compute as
it gets.

\section{Approximation of Areas by
Rectangles}\label{approximation-of-areas-by-rectangles}

Whatever approximation scheme we use, we want it to have the following
properties:

\begin{itemize}
\tightlist
\item
  It uses a geometric figure whose area is easily computable. Rectangles
  are as easy as it gets.
\item
  There is a \textbf{general method} to approximate an area by
  rectangles.
\item
  There is a \textbf{general method} to \textbf{refine} any
  approximation.
\end{itemize}

\subsection{Areas under Curves}\label{areas-under-curves}

Let \(f\colon[a,b]\to\mathbb{R}\) be continuous with \(f(x)\ge0\) for
all \(x\in[a,b]\). We want to compute \[
...
\tightlist
\item
  \textbf{Partition} \([a,b]\) into finitely many segments \[
  a = x_0 < x_1 < \cdots < x_n = b.
  \] The segments \([x_{i-1},x_i]\) will form the \emph{bases} of
  rectangles.
\item
  For each segment \([x_{i-1},x_i]\), pick a point
  \(x_i^*\in[x_{i-1},x_i]\). The \emph{height} of the \(i\)-th rectangle
  is \(f(x_i^*)\).
\end{itemize}

If \(R_i\) denotes the area of the \(i\)-th rectangle, then \[
R_i = \lvert x_i - x_{i-1} \rvert\cdot f(x_i^*) \qquad (i=1,\ldots,n),
\] and \(R_1+\cdots+R_n\) is an approximation to \(A\).

\begin{definition}[]\protect\hypertarget{def-riemann-sum}{}\label{def-riemann-sum}

\hfill\break
A finite set of points \[
P:\quad a = x_0 < x_1 < \cdots < x_n = b
\] is called a \textbf{partition} of \([a,b]\). Points
\(S = \{x_1^*,\ldots,x_n^*\}\) with \(x_i^*\in[x_{i-1},x_i]\) are called
\textbf{sample points}, and \[
\Delta_i = \lvert x_i - x_{i-1} \rvert = x_i - x_{i-1} \qquad (i=1,\ldots,n)
\] is the length of the \(i\)-th segment. The \textbf{Riemann sum} for
\(f\) on \([a,b]\), given \(P\) and \(S\), is \[
...
The symbol \(\sum\) is \textbf{summation} (or \textbf{\(\Sigma\)-})
\textbf{notation}: \(\sum_{i=1}^n c_i\) stands for
\(c_1+c_2+\cdots+c_n\). Note that Definition~\ref{def-riemann-sum} makes
sense for \emph{any} function \(f\) on \([a,b]\) --- neither continuity
nor \(f\ge0\) is needed to \emph{form} the sum. (If \(f\) takes negative
values, the corresponding terms simply count as negative area.)

\textbf{Refinement.} How do we make the approximation better? We
\emph{refine the partition} --- add more division points so that the
segments get shorter --- and choose new sample points. The goal is to
find a way to do this that is both \emph{methodological} and
\emph{practical}. The natural requirement is that the length of
\textbf{every} segment tends to \(0\). It is convenient to measure this
by the length of the longest segment, \[
\|P\| := \max_{1\le i\le n}\Delta_i,
\] called the \textbf{mesh} of \(P\). (Other authors call it the
\emph{span}.)

\section{Equipartitions and the Definition of the
Integral}\label{equipartitions-and-the-definition-of-the-integral}

It turns out the approximation process is remarkably robust --- several
approaches are possible, and they all give the same answer. The most
practical one restricts to a special kind of partition.

\textbf{Equipartitions.} All segments have the \emph{same} length, so
only the number of segments matters. With \(n\) segments, \[
...
\tightlist
\item
  \textbf{left endpoints:} \(x_i^* = x_{i-1}\),
\item
  \textbf{right endpoints:} \(x_i^* = x_i\),
\item
  \textbf{midpoints:} \(x_i^* = \bar x_i := \dfrac{x_{i-1}+x_i}{2}\).
\end{itemize}
...
\] the \textbf{left-hand sum}, the \textbf{right-hand sum}, and the
\textbf{midpoint sum}.

\begin{theorem}[]\protect\hypertarget{thm-riemann-equipartition}{}\label{thm-riemann-equipartition}
...
\begin{definition}[]\protect\hypertarget{def-riemann-integral}{}\label{def-riemann-integral}

\hfill\break
Let \(f\) be continuous on \([a,b]\). The \textbf{(Riemann) integral} of
\(f\) from \(a\) to \(b\) is the common value \(L\) of the limits in
Theorem~\ref{thm-riemann-equipartition}: \[
\int_a^b f(x)\,dx = L = \lim_{n\to\infty} LHS_n = \lim_{n\to\infty} RHS_n = \lim_{n\to\infty} MS_n.
\]

\end{definition}

The notation is Leibniz's: the integral sign \(\int\) is an elongated S
for ``sum'', and \(dx\) recalls the lengths \(\Delta\). It is only
notation --- \(dx\) on its own has no meaning here. Likewise, the name
of the variable is irrelevant:
\(\int_a^b f(x)\,dx = \int_a^b f(t)\,dt\).

\section{Why the Choice Does Not Matter
(optional)}\label{sec-choice-does-not-matter}

Theorem~\ref{thm-riemann-equipartition} is a special case of a much
stronger statement: for a continuous function it does not matter at all
\emph{how} we partition or \emph{where} we sample, as long as the
segments get uniformly short.

\begin{theorem}[Existence of the
integral]\protect\hypertarget{thm-riemann-general}{}\label{thm-riemann-general}

Let \(f\) be continuous on \([a,b]\). There is a unique number \(I\)
with the following property: for \textbf{every} sequence of partitions
\(P_1,P_2,\ldots\) of \([a,b]\) with \(\|P_n\|\to0\), and \textbf{every}
choice of sample points \(S_n\) for \(P_n\), \[
\lim_{n\to\infty} RS(P_n,S_n) = I.
\]

\end{theorem}

Since an equipartition into \(n\) segments has mesh
\(\tfrac{b-a}{n}\to0\), Theorem~\ref{thm-riemann-equipartition} follows
at once, with \(\int_a^b f(x)\,dx = I\). We now sketch \emph{why}
Theorem~\ref{thm-riemann-general} is true. The argument is informal in
two places, flagged in Remark~\ref{rem-darboux-informal} below.

\subsection{Step 1: Trapping Every Riemann
Sum}\label{step-1-trapping-every-riemann-sum}

A continuous function on a closed interval attains a maximum and a
minimum there. (This is the \emph{Extreme Value Theorem}, which we have
not proved yet.) So for a partition \(P\) we can put \[
m_i = \min\{f(x) : x_{i-1}\le x\le x_i\}, \qquad
M_i = \max\{f(x) : x_{i-1}\le x\le x_i\},
\] and form the \textbf{lower} and \textbf{upper (Darboux) sums} \[
L(P) = \sum_{i=1}^n m_i\,\Delta_i, \qquad\qquad U(P) = \sum_{i=1}^n M_i\,\Delta_i.
\] Geometrically (for \(f\ge0\)), \(L(P)\) is the area of the largest
rectangles on the segments that fit \emph{under} the graph, and \(U(P)\)
is the area of the smallest rectangles that reach \emph{over} it.

Whatever sample points \(x_i^*\) we choose, \(m_i\le f(x_i^*)\le M_i\),
so \[
L(P) \;\le\; RS(P,S) \;\le\; U(P) \qquad\text{for every choice of sample points } S.
\] In other words, \(L(P)\) and \(U(P)\) are the \emph{worst cases} over
all choices of sample points. If we can show that they are close to each
other, the choice of sample points becomes irrelevant.

\subsection{Step 2: Refining Moves the Sums Toward Each
Other}\label{step-2-refining-moves-the-sums-toward-each-other}

Suppose \(P'\) is \textbf{finer} than \(P\), i.e.~\(P'\) contains all
points of \(P\) and possibly more. Adding a single point \(z\) inside a
segment \([x_{i-1},x_i]\) splits it into \([x_{i-1},z]\) and
\([z,x_i]\). The minimum of \(f\) over each smaller piece is \emph{at
least} the minimum \(m_i\) over the whole segment, and the maximum over
each piece is \emph{at most} \(M_i\). Adding points one at a time, we
get \[
L(P) \le L(P') \qquad\text{and}\qquad U(P') \le U(P).
\] So refining can only push the lower sum up and the upper sum down.

This has a useful consequence. Given \emph{any} two partitions \(P\) and
\(Q\), the partition \(P\cup Q\) is finer than both, so \[
L(P) \;\le\; L(P\cup Q) \;\le\; U(P\cup Q) \;\le\; U(Q).
\] That is: \textbf{every lower sum is at most every upper sum}, no
matter which partitions they come from. All lower sums sit to the left
of all upper sums on the number line, and by the completeness of
\(\mathbb{R}\) (compare the Nested Interval Axiom) there is at least one
number \(I\) in between: \[
L(P) \le I \le U(Q) \qquad\text{for all partitions } P, Q.
\]

\subsection{Step 3: Closing the Gap}\label{step-3-closing-the-gap}

The gap between upper and lower sum is \[
U(P) - L(P) = \sum_{i=1}^n (M_i - m_i)\,\Delta_i.
\] Here continuity enters. On a very short segment a continuous function
cannot vary much. Suppose we know the following: for every
\(\varepsilon>0\) there is a \(\delta>0\) such that
\begin{equation}\protect\phantomsection\label{eq-uniform-continuity}{
\lvert x-y \rvert<\delta \quad \Rightarrow \quad\lvert f(x)-f(y) \rvert<\varepsilon\qquad\text{for all } x,y\in[a,b].
}\end{equation} Then, as soon as \(\|P\|<\delta\), any two points in the
same segment are within \(\delta\) of each other, so
\(M_i - m_i<\varepsilon\) for every \(i\), and \[
U(P) - L(P) < \sum_{i=1}^n \varepsilon\,\Delta_i = \varepsilon\,(b-a).
\] Since \(\varepsilon\) was arbitrary, the gap can be made as small as
we like by making the mesh small.

\subsection{Step 4: Conclusion}\label{step-4-conclusion}

Now let \(\|P_n\|\to0\) and let \(S_n\) be any sample points. By Steps 1
and 2, both \(RS(P_n,S_n)\) and \(I\) lie in the interval
\([L(P_n),U(P_n)]\), so \[
\lvert RS(P_n,S_n) - I \rvert \le U(P_n) - L(P_n).
\] By Step 3 the right-hand side tends to \(0\), so \(RS(P_n,S_n)\to I\)
(by the comparison test, Proposition~\ref{prp-comparison}). The number
\(I\) is unique: any other \(I'\) trapped between all lower and all
upper sums satisfies \(\lvert I-I' \rvert\le U(P_n)-L(P_n)\to0\) as
well. Nothing in this argument depended on \emph{which} partitions or
sample points we used --- only on the mesh going to \(0\).
...
\tightlist
\item
  the \textbf{Extreme Value Theorem} (continuous functions attain a max
  and min on closed intervals), used to define \(m_i\) and \(M_i\);
\item
  the existence of a number \(I\) between all lower and all upper sums,
  which is a consequence of the completeness of \(\mathbb{R}\);
\item
  condition Equation~\ref{eq-uniform-continuity}, called \textbf{uniform
  continuity}.
\end{enumerate}

The third point is the subtle one. It looks like
Definition~\ref{def-continuity}, but the quantifiers are in a different
order: in Definition~\ref{def-continuity}, \(\delta\) is chosen
\emph{after} the point \(a\), so it may depend on \(a\); in
Equation~\ref{eq-uniform-continuity}, one \(\delta\) must work
\emph{simultaneously for all points} of \([a,b]\). For instance,
\(f(x)=\tfrac1x\) is continuous on \((0,1]\), but near \(0\) it becomes
steeper and steeper, and no single \(\delta\) works for a given
\(\varepsilon\). On a \textbf{closed, bounded} interval this cannot
happen: a continuous \(f\colon[a,b]\to\mathbb{R}\) is automatically
uniformly continuous. Like the Extreme Value Theorem, this can be proved
with a bisection / nested interval argument in the spirit of the proof
of Theorem~\ref{thm-ivt}.

\label{rem-darboux-informal}

\end{refremark}

\begin{refremark}
Continuity really matters. For the Dirichlet function of
Example~\ref{exm-dirichlet-discontinuous} on \([0,1]\), every segment of
every partition contains both rational and irrational points. Choosing
all sample points rational gives
\(RS(P,S) = \sum_i 1\cdot\Delta_i = 1\); choosing them all irrational
gives \(RS(P,S)=0\) --- for \emph{every} partition, however fine. So the
Riemann sums have no common limit, and the Dirichlet function is not
Riemann integrable.

\label{rem-dirichlet-not-integrable}

\end{refremark}

\section{Verifying the Riemann
Integral}\label{verifying-the-riemann-integral}

Before computing anything new, let us check that
Definition~\ref{def-riemann-integral} gives the right answer for areas
we already know from geometry. By
Theorem~\ref{thm-riemann-equipartition} we may use whichever of the
three sums is most convenient. Throughout, \(\Delta = \frac{b-a}{n}\)
and \(x_i = a + i\Delta\).

\begin{example}[]\protect\hypertarget{exm-integral-constant}{}\label{exm-integral-constant}
...
\begin{example}[]\protect\hypertarget{exm-integral-x}{}\label{exm-integral-x}
...
\end{aligned}
\] Here we used \(1+2+\cdots+n = \frac{n(n+1)}{2}\) and
\(\frac{n+1}{n}\to1\).

For \(0\le a<b\) this is exactly what geometry predicts: the region
under \(y=x\) between \(a\) and \(b\) is a rectangle of base \(b-a\) and
height \(a\) (area \((b-a)\,a\)), topped by a right triangle with both
legs of length \(b-a\) (area \(\frac{(b-a)^2}{2}\)). ✓ Simplifying, \[
\int_a^b x\,dx = (b-a)\Big(a + \frac{b-a}{2}\Big) = \frac{(b-a)(b+a)}{2} = \frac{b^2-a^2}{2}.
\]

\end{example}

\begin{exercise}[]\protect\hypertarget{exr-integral-linear}{}\label{exr-integral-linear}

Verify this for an arbitrary linear function \(f(x) = mx + c\): compute
\(\lim_{n\to\infty}RHS_n\) and check that the result agrees with the
area of the trapezoid under the graph (when \(f\ge0\) on \([a,b]\)).

\end{exercise}

\section{\texorpdfstring{``New'' Functions:
\(x^2\)}{``New'' Functions: x\^{}2}}\label{new-functions-x2}

The examples above only reproduced areas we already knew. The first
genuinely new area is the one under a parabola. Let \(f(x) = x^2\). Then
\[
...
\begin{exercise}[]\protect\hypertarget{exr-sum-of-squares}{}\label{exr-sum-of-squares}
...
\begin{exercise}[]\protect\hypertarget{exr-integral-x-squared-general}{}\label{exr-integral-x-squared-general}

Generalize the computation above to show that for arbitrary \(a<b\), \[
\int_a^b x^2\,dx = \frac{b^3-a^3}{3}.
\] (Expand \(\big(a+\frac{b-a}{n}\,i\big)^2\) and use the formulas for
\(\sum i\) and \(\sum i^2\).)
...
\tightlist
\item
  The area of the unit circle is the limit of the areas
  \(A_n = \frac n2\sin\!\big(\frac{2\pi}{n}\big)\) of inscribed regular
  \(n\)-gons; the sequence is increasing and bounded, so it converges by
  ...
  \end{enumerate}
\item
  For areas under a graph we use \textbf{rectangles}: a
  \textbf{partition} \(P\) of \([a,b]\) and \textbf{sample points} \(S\)
  give the \textbf{Riemann sum}
  \(RS(P,S) = \sum_{i=1}^n f(x_i^*)\,\Delta_i\)
  (Definition~\ref{def-riemann-sum}). We improve the approximation by
  refining the partition so that the \textbf{mesh}
  \(\|P\|=\max_i\Delta_i\) tends to \(0\).
\item
  Restricting to \textbf{equipartitions} with left endpoints, right
  endpoints, or midpoints gives three sequences \(LHS_n\), \(RHS_n\),
  \(MS_n\). For continuous \(f\) they have a common limit
  (Theorem~\ref{thm-riemann-equipartition}), which \emph{defines}
  \(\int_a^b f(x)\,dx\) (Definition~\ref{def-riemann-integral}).
\item
  More generally, for continuous \(f\) \textbf{every} sequence of
  Riemann sums with mesh \(\to0\) converges to the same number
  (Theorem~\ref{thm-riemann-general}). The idea: every Riemann sum is
  trapped between the \textbf{lower and upper sums}
  \(L(P)\le RS(P,S)\le U(P)\); refining pushes these toward each other;
  and \textbf{uniform continuity} makes the gap \(U(P)-L(P)\) smaller
  than \(\varepsilon(b-a)\) once the mesh is small
  (Remark~\ref{rem-darboux-informal}). Without continuity this can fail,
  as the Dirichlet function shows
  (Remark~\ref{rem-dirichlet-not-integrable}).
\item
  The definition reproduces known areas --- \(\int_a^b c\,dx = (b-a)c\)
  (Example~\ref{exm-integral-constant}) and
  \(\int_a^b x\,dx = \frac{b^2-a^2}{2}\) (Example~\ref{exm-integral-x})
  --- and computes new ones: \(\int_0^b x^2\,dx = \frac{b^3}{3}\).
\end{itemize}

\chapter{Lab: Riemann Sums}\label{lab-riemann-sums}

Math 140H, Honors Calculus I

In class we approximated the area under a curve by rectangles. This
notebook lets you build those rectangles, look at them, and watch the
Riemann sums approach the integral as the partition gets finer.

How to use this notebook - Cells marked IN CLASS are the core we will do
together. - Cells marked HOMEWORK extend the same ideas further. - Cells
marked PREDICT ask you to write down a guess before running the next
code cell. Do not skip these; the point is to notice when your intuition
is wrong.

\textbf{START BY RUNNNING THE NEXT CELL}, this will load the necessary
libraries and define all Python functions needed.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#}
\CommentTok{\#}
\CommentTok{\# ALWAYS RUN THIS CELL FIRST!!}
\CommentTok{\#}
\CommentTok{\#}
...
\NormalTok{plt.rcParams[}\StringTok{\textquotesingle{}figure.figsize\textquotesingle{}}\NormalTok{] }\OperatorTok{=}\NormalTok{ (}\DecValTok{8}\NormalTok{, }\FloatTok{4.5}\NormalTok{)}
\NormalTok{x }\OperatorTok{=}\NormalTok{ sp.Symbol(}\StringTok{\textquotesingle{}x\textquotesingle{}}\NormalTok{)}

\NormalTok{\_METHODS }\OperatorTok{=}\NormalTok{ \{}
    \StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}l\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}lefthand\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}left{-}hand\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{,}
    \StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}r\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}righthand\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}right{-}hand\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{,}
    \StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}mid\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}m\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}middle\textquotesingle{}}\NormalTok{: }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{,}
\NormalTok{\}}


\KeywordTok{def}\NormalTok{ \_to\_exact(v):}
    \CommentTok{"""Turn a number into an exact sympy number (0.25 {-}\textgreater{} 1/4, pi stays pi)."""}
\NormalTok{    v }\OperatorTok{=}\NormalTok{ sp.sympify(v)}
    \ControlFlowTok{if} \BuiltInTok{isinstance}\NormalTok{(v, sp.Float):}
\NormalTok{        v }\OperatorTok{=}\NormalTok{ sp.nsimplify(v, rational}\OperatorTok{=}\VariableTok{True}\NormalTok{)}
    \ControlFlowTok{return}\NormalTok{ v}


\KeywordTok{def}\NormalTok{ \_fmt(v, digits}\OperatorTok{=}\DecValTok{6}\NormalTok{):}
    \CommentTok{"""Format a float compactly, e.g. 0.0625, 1.41421, {-}3.2e{-}05."""}
\NormalTok{    s }\OperatorTok{=} \SpecialStringTok{f\textquotesingle{}}\SpecialCharTok{\{}\BuiltInTok{float}\NormalTok{(v)}\SpecialCharTok{:.}\NormalTok{\{digits\}g}\SpecialCharTok{\}}\SpecialStringTok{\textquotesingle{}}
    \ControlFlowTok{if} \StringTok{\textquotesingle{}e\textquotesingle{}} \KeywordTok{in}\NormalTok{ s:}
\NormalTok{        mant, ex }\OperatorTok{=}\NormalTok{ s.split(}\StringTok{\textquotesingle{}e\textquotesingle{}}\NormalTok{)}
\NormalTok{        s }\OperatorTok{=} \VerbatimStringTok{rf\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{mant}\SpecialCharTok{\}}\VerbatimStringTok{ }\ErrorTok{\textbackslash{}}\VerbatimStringTok{cdot 10}\DecValTok{\^{}}\CharTok{\{\{}\SpecialCharTok{\{}\BuiltInTok{int}\NormalTok{(ex)}\SpecialCharTok{\}}\CharTok{\}\}}\VerbatimStringTok{\textquotesingle{}}
    \ControlFlowTok{return}\NormalTok{ s}


\KeywordTok{def}\NormalTok{ \_join\_terms(terms, n, max\_terms):}
    \CommentTok{"""Join LaTeX terms with + and {-} signs; if n is large, abbreviate with + ... +."""}
    \ControlFlowTok{if}\NormalTok{ n }\OperatorTok{\textgreater{}}\NormalTok{ max\_terms:}
\NormalTok{        terms }\OperatorTok{=}\NormalTok{ terms[:}\DecValTok{3}\NormalTok{] }\OperatorTok{+}\NormalTok{ [}\VerbatimStringTok{r\textquotesingle{}}\ErrorTok{\textbackslash{}}\VerbatimStringTok{cdots\textquotesingle{}}\NormalTok{] }\OperatorTok{+}\NormalTok{ terms[}\OperatorTok{{-}}\DecValTok{2}\NormalTok{:]}
\NormalTok{    out }\OperatorTok{=}\NormalTok{ terms[}\DecValTok{0}\NormalTok{]}
    \ControlFlowTok{for}\NormalTok{ t }\KeywordTok{in}\NormalTok{ terms[}\DecValTok{1}\NormalTok{:]:}
        \ControlFlowTok{if}\NormalTok{ t.startswith(}\StringTok{\textquotesingle{}{-}\textquotesingle{}}\NormalTok{):}
\NormalTok{            out }\OperatorTok{+=} \StringTok{\textquotesingle{} {-} \textquotesingle{}} \OperatorTok{+}\NormalTok{ t[}\DecValTok{1}\NormalTok{:].lstrip()}
        \ControlFlowTok{else}\NormalTok{:}
\NormalTok{            out }\OperatorTok{+=} \StringTok{\textquotesingle{} + \textquotesingle{}} \OperatorTok{+}\NormalTok{ t}
    \ControlFlowTok{return}\NormalTok{ out}


\KeywordTok{def}\NormalTok{ riemann\_sum(f, a, b, n, method}\OperatorTok{=}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, x}\OperatorTok{=}\VariableTok{None}\NormalTok{, show}\OperatorTok{=}\VariableTok{True}\NormalTok{,}
\NormalTok{                show\_integral}\OperatorTok{=}\VariableTok{False}\NormalTok{, max\_terms}\OperatorTok{=}\DecValTok{6}\NormalTok{):}
    \CommentTok{"""}
\CommentTok{    Compute (and by default draw) a Riemann sum for f on [a, b].}

\CommentTok{    Parameters}
\CommentTok{    {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\CommentTok{    f : sympy expression in one variable, e.g. x**2 or sp.sin(x)}
\CommentTok{    a, b : the integration bounds, a \textless{} b (numbers or sympy numbers like sp.pi)}
\CommentTok{    n : number of subintervals of equal length}
\CommentTok{    method : \textquotesingle{}left\textquotesingle{}, \textquotesingle{}right\textquotesingle{}, or \textquotesingle{}midpoint\textquotesingle{}}
\CommentTok{    x : the variable of f (only needed if f has more than one symbol)}
\CommentTok{    show : if False, just return the value (no picture, no computation lines)}
\CommentTok{    show\_integral : also display the exact integral and the error of the sum}
\CommentTok{    max\_terms : largest n for which every term of the sum is written out;}
\CommentTok{                for larger n the sum is abbreviated with "+ ... +"}

\CommentTok{    Returns}
\CommentTok{    {-}{-}{-}{-}{-}{-}{-}}
\CommentTok{    The value of the Riemann sum as a float.}
\CommentTok{    """}
    \CommentTok{\# {-}{-}{-}{-} check and prepare the input {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{    method }\OperatorTok{=}\NormalTok{ \_METHODS.get(}\BuiltInTok{str}\NormalTok{(method).lower().replace(}\StringTok{\textquotesingle{} \textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{))}
    \ControlFlowTok{if}\NormalTok{ method }\KeywordTok{is} \VariableTok{None}\NormalTok{:}
        \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\StringTok{"method must be \textquotesingle{}left\textquotesingle{}, \textquotesingle{}right\textquotesingle{}, or \textquotesingle{}midpoint\textquotesingle{}"}\NormalTok{)}
\NormalTok{    n }\OperatorTok{=} \BuiltInTok{int}\NormalTok{(n)}
    \ControlFlowTok{if}\NormalTok{ n }\OperatorTok{\textless{}} \DecValTok{1}\NormalTok{:}
        \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\StringTok{\textquotesingle{}n must be a positive integer\textquotesingle{}}\NormalTok{)}
\NormalTok{    f }\OperatorTok{=}\NormalTok{ sp.sympify(f)}
    \ControlFlowTok{if}\NormalTok{ x }\KeywordTok{is} \VariableTok{None}\NormalTok{:}
\NormalTok{        syms }\OperatorTok{=} \BuiltInTok{sorted}\NormalTok{(f.free\_symbols, key}\OperatorTok{=}\BuiltInTok{str}\NormalTok{)}
        \ControlFlowTok{if} \BuiltInTok{len}\NormalTok{(syms) }\OperatorTok{\textgreater{}} \DecValTok{1}\NormalTok{:}
            \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\SpecialStringTok{f\textquotesingle{}f has several symbols }\SpecialCharTok{\{}\NormalTok{syms}\SpecialCharTok{\}}\SpecialStringTok{; pass the variable as x=...\textquotesingle{}}\NormalTok{)}
\NormalTok{        x }\OperatorTok{=}\NormalTok{ syms[}\DecValTok{0}\NormalTok{] }\ControlFlowTok{if}\NormalTok{ syms }\ControlFlowTok{else}\NormalTok{ sp.Symbol(}\StringTok{\textquotesingle{}x\textquotesingle{}}\NormalTok{)}
\NormalTok{    a, b }\OperatorTok{=}\NormalTok{ \_to\_exact(a), \_to\_exact(b)}
    \ControlFlowTok{if} \KeywordTok{not}\NormalTok{ (sp.N(a) }\OperatorTok{\textless{}}\NormalTok{ sp.N(b)):}
        \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\StringTok{\textquotesingle{}need a \textless{} b\textquotesingle{}}\NormalTok{)}

    \CommentTok{\# {-}{-}{-}{-} the partition and the sample points {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{    delta }\OperatorTok{=}\NormalTok{ (b }\OperatorTok{{-}}\NormalTok{ a) }\OperatorTok{/}\NormalTok{ n                    }\CommentTok{\# exact length of each subinterval}
\NormalTok{    shift }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{: }\DecValTok{0}\NormalTok{, }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{: sp.Rational(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{)\}[method]}
\NormalTok{    offset }\OperatorTok{=}\NormalTok{ \{}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{: }\DecValTok{1}\NormalTok{, }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{: }\DecValTok{0}\NormalTok{, }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{: sp.Rational(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{)\}[method]}
    \CommentTok{\# sample point x\_i\^{}* = a + (i {-} offset) * delta, for i = 1, ..., n}

\NormalTok{    f\_num }\OperatorTok{=}\NormalTok{ sp.lambdify(x, f, }\StringTok{\textquotesingle{}numpy\textquotesingle{}}\NormalTok{)}
\NormalTok{    a\_f, b\_f, d\_f }\OperatorTok{=} \BuiltInTok{float}\NormalTok{(a), }\BuiltInTok{float}\NormalTok{(b), }\BuiltInTok{float}\NormalTok{(delta)}
\NormalTok{    left\_edges }\OperatorTok{=}\NormalTok{ a\_f }\OperatorTok{+}\NormalTok{ d\_f }\OperatorTok{*}\NormalTok{ np.arange(n)}
\NormalTok{    samples }\OperatorTok{=}\NormalTok{ left\_edges }\OperatorTok{+} \BuiltInTok{float}\NormalTok{(shift) }\OperatorTok{*}\NormalTok{ d\_f}
    \ControlFlowTok{with}\NormalTok{ np.errstate(}\BuiltInTok{all}\OperatorTok{=}\StringTok{\textquotesingle{}ignore\textquotesingle{}}\NormalTok{):}
\NormalTok{        heights }\OperatorTok{=}\NormalTok{ np.broadcast\_to(np.asarray(f\_num(samples), dtype}\OperatorTok{=}\BuiltInTok{float}\NormalTok{), samples.shape)}
    \ControlFlowTok{if} \KeywordTok{not}\NormalTok{ np.}\BuiltInTok{all}\NormalTok{(np.isfinite(heights)):}
\NormalTok{        bad }\OperatorTok{=}\NormalTok{ samples[}\OperatorTok{\textasciitilde{}}\NormalTok{np.isfinite(heights)][}\DecValTok{0}\NormalTok{]}
        \ControlFlowTok{raise} \PreprocessorTok{ValueError}\NormalTok{(}\SpecialStringTok{f\textquotesingle{}f is not defined (or infinite) at the sample point }\SpecialCharTok{\{}\NormalTok{bad}\SpecialCharTok{:g\}}\SpecialStringTok{; \textquotesingle{}}
                         \StringTok{\textquotesingle{}try a different method or interval\textquotesingle{}}\NormalTok{)}
\NormalTok{    value }\OperatorTok{=} \BuiltInTok{float}\NormalTok{(np.}\BuiltInTok{sum}\NormalTok{(heights) }\OperatorTok{*}\NormalTok{ d\_f)}
    \ControlFlowTok{if} \BuiltInTok{abs}\NormalTok{(value) }\OperatorTok{\textless{}} \FloatTok{1e{-}12} \OperatorTok{*} \BuiltInTok{max}\NormalTok{(}\FloatTok{1.0}\NormalTok{, }\BuiltInTok{float}\NormalTok{(np.}\BuiltInTok{sum}\NormalTok{(np.}\BuiltInTok{abs}\NormalTok{(heights)) }\OperatorTok{*}\NormalTok{ d\_f)):}
\NormalTok{        value }\OperatorTok{=} \FloatTok{0.0}          \CommentTok{\# the exact answer is 0; the rest is rounding noise}

    \ControlFlowTok{if} \KeywordTok{not}\NormalTok{ show:}
        \ControlFlowTok{return}\NormalTok{ value}

    \CommentTok{\# {-}{-}{-}{-} the picture {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{    fig, ax }\OperatorTok{=}\NormalTok{ plt.subplots()}
\NormalTok{    colors }\OperatorTok{=}\NormalTok{ np.where(heights }\OperatorTok{\textgreater{}=} \DecValTok{0}\NormalTok{, }\StringTok{\textquotesingle{}tab:blue\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}tab:red\textquotesingle{}}\NormalTok{)}
\NormalTok{    edge }\OperatorTok{=} \StringTok{\textquotesingle{}black\textquotesingle{}} \ControlFlowTok{if}\NormalTok{ n }\OperatorTok{\textless{}=} \DecValTok{100} \ControlFlowTok{else} \StringTok{\textquotesingle{}none\textquotesingle{}}
\NormalTok{    ax.bar(left\_edges, heights, width}\OperatorTok{=}\NormalTok{d\_f, align}\OperatorTok{=}\StringTok{\textquotesingle{}edge\textquotesingle{}}\NormalTok{, color}\OperatorTok{=}\NormalTok{colors,}
\NormalTok{           alpha}\OperatorTok{=}\FloatTok{0.35}\NormalTok{, edgecolor}\OperatorTok{=}\NormalTok{edge, linewidth}\OperatorTok{=}\FloatTok{0.8}\NormalTok{)}
\NormalTok{    xs }\OperatorTok{=}\NormalTok{ np.linspace(a\_f, b\_f, }\DecValTok{1000}\NormalTok{)}
    \ControlFlowTok{with}\NormalTok{ np.errstate(}\BuiltInTok{all}\OperatorTok{=}\StringTok{\textquotesingle{}ignore\textquotesingle{}}\NormalTok{):}
\NormalTok{        ys }\OperatorTok{=}\NormalTok{ np.array(np.broadcast\_to(np.asarray(f\_num(xs), dtype}\OperatorTok{=}\BuiltInTok{float}\NormalTok{), xs.shape))}
\NormalTok{    ys[}\OperatorTok{\textasciitilde{}}\NormalTok{np.isfinite(ys)] }\OperatorTok{=}\NormalTok{ np.nan}
\NormalTok{    ax.plot(xs, ys, color}\OperatorTok{=}\StringTok{\textquotesingle{}black\textquotesingle{}}\NormalTok{, linewidth}\OperatorTok{=}\DecValTok{2}\NormalTok{, label}\OperatorTok{=}\SpecialStringTok{f\textquotesingle{}$f(}\SpecialCharTok{\{}\NormalTok{x}\SpecialCharTok{\}}\SpecialStringTok{) = }\SpecialCharTok{\{}\NormalTok{sp}\SpecialCharTok{.}\NormalTok{latex(f)}\SpecialCharTok{\}}\SpecialStringTok{$\textquotesingle{}}\NormalTok{)}
    \ControlFlowTok{if}\NormalTok{ n }\OperatorTok{\textless{}=} \DecValTok{60}\NormalTok{:}
\NormalTok{        ax.plot(samples, heights, }\StringTok{\textquotesingle{}o\textquotesingle{}}\NormalTok{, color}\OperatorTok{=}\StringTok{\textquotesingle{}darkorange\textquotesingle{}}\NormalTok{, markersize}\OperatorTok{=}\DecValTok{5}\NormalTok{,}
\NormalTok{                label}\OperatorTok{=}\VerbatimStringTok{r\textquotesingle{}}\DecValTok{$}\KeywordTok{(}\VerbatimStringTok{x\_i}\DecValTok{\^{}}\OperatorTok{*}\VerbatimStringTok{, f}\KeywordTok{(}\VerbatimStringTok{x\_i}\DecValTok{\^{}}\OperatorTok{*}\KeywordTok{))}\DecValTok{$}\VerbatimStringTok{\textquotesingle{}}\NormalTok{)}
\NormalTok{    ax.axhline(}\DecValTok{0}\NormalTok{, color}\OperatorTok{=}\StringTok{\textquotesingle{}gray\textquotesingle{}}\NormalTok{, linewidth}\OperatorTok{=}\FloatTok{0.8}\NormalTok{)}
\NormalTok{    pad }\OperatorTok{=} \FloatTok{0.05} \OperatorTok{*}\NormalTok{ (b\_f }\OperatorTok{{-}}\NormalTok{ a\_f)}
\NormalTok{    ax.set\_xlim(a\_f }\OperatorTok{{-}}\NormalTok{ pad, b\_f }\OperatorTok{+}\NormalTok{ pad)}
    \CommentTok{\# if f blows up somewhere, keep the rectangles in view instead of the spike}
\NormalTok{    h\_lo, h\_hi }\OperatorTok{=} \BuiltInTok{min}\NormalTok{(}\FloatTok{0.0}\NormalTok{, heights.}\BuiltInTok{min}\NormalTok{()), }\BuiltInTok{max}\NormalTok{(}\FloatTok{0.0}\NormalTok{, heights.}\BuiltInTok{max}\NormalTok{())}
\NormalTok{    span }\OperatorTok{=}\NormalTok{ h\_hi }\OperatorTok{{-}}\NormalTok{ h\_lo}
\NormalTok{    y\_lo, y\_hi }\OperatorTok{=}\NormalTok{ np.nanmin(ys), np.nanmax(ys)}
    \ControlFlowTok{if}\NormalTok{ span }\OperatorTok{\textgreater{}} \DecValTok{0} \KeywordTok{and}\NormalTok{ (y\_hi }\OperatorTok{\textgreater{}}\NormalTok{ h\_hi }\OperatorTok{+} \DecValTok{2} \OperatorTok{*}\NormalTok{ span }\KeywordTok{or}\NormalTok{ y\_lo }\OperatorTok{\textless{}}\NormalTok{ h\_lo }\OperatorTok{{-}} \DecValTok{2} \OperatorTok{*}\NormalTok{ span):}
\NormalTok{        lo, hi }\OperatorTok{=} \BuiltInTok{max}\NormalTok{(y\_lo, h\_lo }\OperatorTok{{-}} \FloatTok{0.5} \OperatorTok{*}\NormalTok{ span), }\BuiltInTok{min}\NormalTok{(y\_hi, h\_hi }\OperatorTok{+} \FloatTok{0.5} \OperatorTok{*}\NormalTok{ span)}
\NormalTok{        ax.set\_ylim(}\BuiltInTok{min}\NormalTok{(lo, h\_lo) }\OperatorTok{{-}} \FloatTok{0.05} \OperatorTok{*}\NormalTok{ span, }\BuiltInTok{max}\NormalTok{(hi, h\_hi) }\OperatorTok{+} \FloatTok{0.05} \OperatorTok{*}\NormalTok{ span)}
\NormalTok{    ax.set\_xlabel(}\SpecialStringTok{f\textquotesingle{}$}\SpecialCharTok{\{}\NormalTok{x}\SpecialCharTok{\}}\SpecialStringTok{$\textquotesingle{}}\NormalTok{)}
\NormalTok{    ax.set\_title(}\SpecialStringTok{f\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{method}\SpecialCharTok{.}\NormalTok{capitalize()}\SpecialCharTok{\}}\SpecialStringTok{ Riemann sum with n = }\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\SpecialStringTok{\textquotesingle{}}\NormalTok{)}
\NormalTok{    handles, labels }\OperatorTok{=}\NormalTok{ ax.get\_legend\_handles\_labels()}
    \ControlFlowTok{if}\NormalTok{ np.}\BuiltInTok{any}\NormalTok{(heights }\OperatorTok{\textless{}} \DecValTok{0}\NormalTok{):}
\NormalTok{        handles }\OperatorTok{+=}\NormalTok{ [Patch(color}\OperatorTok{=}\StringTok{\textquotesingle{}tab:blue\textquotesingle{}}\NormalTok{, alpha}\OperatorTok{=}\FloatTok{0.35}\NormalTok{), Patch(color}\OperatorTok{=}\StringTok{\textquotesingle{}tab:red\textquotesingle{}}\NormalTok{, alpha}\OperatorTok{=}\FloatTok{0.35}\NormalTok{)]}
\NormalTok{        labels }\OperatorTok{+=}\NormalTok{ [}\VerbatimStringTok{r\textquotesingle{}}\DecValTok{$}\VerbatimStringTok{f}\KeywordTok{(}\VerbatimStringTok{x\_i}\DecValTok{\^{}}\OperatorTok{*}\KeywordTok{)}\VerbatimStringTok{ }\ErrorTok{\textbackslash{}}\VerbatimStringTok{geq 0}\DecValTok{$}\VerbatimStringTok{\textquotesingle{}}\NormalTok{, }\VerbatimStringTok{r\textquotesingle{}}\DecValTok{$}\VerbatimStringTok{f}\KeywordTok{(}\VerbatimStringTok{x\_i}\DecValTok{\^{}}\OperatorTok{*}\KeywordTok{)}\VerbatimStringTok{ \textless{} 0}\DecValTok{$}\VerbatimStringTok{\textquotesingle{}}\NormalTok{]}
\NormalTok{    ax.legend(handles, labels, loc}\OperatorTok{=}\StringTok{\textquotesingle{}best\textquotesingle{}}\NormalTok{)}
\NormalTok{    ax.grid(}\VariableTok{True}\NormalTok{, alpha}\OperatorTok{=}\FloatTok{0.3}\NormalTok{)}
\NormalTok{    plt.show()}

    \CommentTok{\# {-}{-}{-}{-} the computation, line by line {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{    A, B, D }\OperatorTok{=}\NormalTok{ sp.latex(a), sp.latex(b), sp.latex(delta)}
\NormalTok{    fx }\OperatorTok{=}\NormalTok{ sp.latex(f)}
\NormalTok{    sample\_rule }\OperatorTok{=}\NormalTok{ \{}
        \StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{:     }\VerbatimStringTok{r\textquotesingle{}x\_i}\DecValTok{\^{}}\OperatorTok{*}\VerbatimStringTok{ = x\_\{i{-}1\} = a }\OperatorTok{+}\VerbatimStringTok{ }\KeywordTok{(}\VerbatimStringTok{i{-}1}\KeywordTok{)}\DecValTok{\textbackslash{}D}\VerbatimStringTok{elta\textquotesingle{}}\NormalTok{,}
        \StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{:    }\VerbatimStringTok{r\textquotesingle{}x\_i}\DecValTok{\^{}}\OperatorTok{*}\VerbatimStringTok{ = x\_i = a }\OperatorTok{+}\VerbatimStringTok{ i}\CharTok{\textbackslash{},}\DecValTok{\textbackslash{}D}\VerbatimStringTok{elta\textquotesingle{}}\NormalTok{,}
        \StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{: }\VerbatimStringTok{r\textquotesingle{}x\_i}\DecValTok{\^{}}\OperatorTok{*}\VerbatimStringTok{ = }\CharTok{\textbackslash{}f}\VerbatimStringTok{rac\{x\_\{i{-}1\} }\OperatorTok{+}\VerbatimStringTok{ x\_i\}}\OperatorTok{\{2\}}\VerbatimStringTok{ = a }\OperatorTok{+}\VerbatimStringTok{ }\ErrorTok{\textbackslash{}}\VerbatimStringTok{left}\KeywordTok{(}\VerbatimStringTok{i {-} }\CharTok{\textbackslash{}t}\VerbatimStringTok{frac}\OperatorTok{\{1\}\{2\}}\CharTok{\textbackslash{}r}\VerbatimStringTok{ight}\KeywordTok{)}\DecValTok{\textbackslash{}D}\VerbatimStringTok{elta\textquotesingle{}}\NormalTok{,}
\NormalTok{    \}[method]}

\NormalTok{    display(Math(}\VerbatimStringTok{rf\textquotesingle{}f}\KeywordTok{(}\SpecialCharTok{\{}\NormalTok{x}\SpecialCharTok{\}}\KeywordTok{)}\VerbatimStringTok{ = }\SpecialCharTok{\{}\NormalTok{fx}\SpecialCharTok{\}}\VerbatimStringTok{, }\ErrorTok{\textbackslash{}}\VerbatimStringTok{qquad }\PreprocessorTok{[a, b]}\VerbatimStringTok{ = }\PreprocessorTok{[}\SpecialCharTok{\{}\NormalTok{A}\SpecialCharTok{\}}\PreprocessorTok{, }\SpecialCharTok{\{}\NormalTok{B}\SpecialCharTok{\}}\PreprocessorTok{]}\VerbatimStringTok{, }\ErrorTok{\textbackslash{}}\VerbatimStringTok{qquad n = }\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}
\NormalTok{    a\_paren }\OperatorTok{=} \VerbatimStringTok{rf\textquotesingle{}}\ErrorTok{\textbackslash{}}\VerbatimStringTok{left}\KeywordTok{(}\SpecialCharTok{\{}\NormalTok{A}\SpecialCharTok{\}}\CharTok{\textbackslash{}r}\VerbatimStringTok{ight}\KeywordTok{)}\VerbatimStringTok{\textquotesingle{}} \ControlFlowTok{if}\NormalTok{ sp.N(a) }\OperatorTok{\textless{}} \DecValTok{0} \ControlFlowTok{else}\NormalTok{ A}
\NormalTok{    display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\DecValTok{\textbackslash{}D}\VerbatimStringTok{elta = }\CharTok{\textbackslash{}f}\VerbatimStringTok{rac}\CharTok{\{\{}\VerbatimStringTok{b{-}a}\CharTok{\}\}\{\{}\VerbatimStringTok{n}\CharTok{\}\}}\VerbatimStringTok{ = }\CharTok{\textbackslash{}f}\VerbatimStringTok{rac}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{B}\SpecialCharTok{\}}\VerbatimStringTok{ {-} }\SpecialCharTok{\{}\NormalTok{a\_paren}\SpecialCharTok{\}}\CharTok{\}\}\{\{}\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\CharTok{\}\}}\VerbatimStringTok{ = }\SpecialCharTok{\{}\NormalTok{D}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}}
                 \OperatorTok{+}\NormalTok{ (}\VerbatimStringTok{rf\textquotesingle{} }\CharTok{\textbackslash{}a}\VerbatimStringTok{pprox }\SpecialCharTok{\{}\NormalTok{\_fmt(d\_f)}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}} \ControlFlowTok{if} \KeywordTok{not}\NormalTok{ delta.is\_Rational }\ControlFlowTok{else} \StringTok{\textquotesingle{}\textquotesingle{}}\NormalTok{)}
                 \OperatorTok{+} \VerbatimStringTok{rf\textquotesingle{}, }\ErrorTok{\textbackslash{}}\VerbatimStringTok{qquad x\_i = a }\OperatorTok{+}\VerbatimStringTok{ i}\CharTok{\textbackslash{},}\DecValTok{\textbackslash{}D}\VerbatimStringTok{elta }\ErrorTok{\textbackslash{}}\VerbatimStringTok{quad }\KeywordTok{(}\VerbatimStringTok{i = 0, 1, }\ErrorTok{\textbackslash{}}\VerbatimStringTok{ldots, }\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\KeywordTok{)}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}
\NormalTok{    display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\CharTok{\textbackslash{}t}\VerbatimStringTok{ext}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{method}\SpecialCharTok{\}}\VerbatimStringTok{ sample points: }\CharTok{\}\}}\VerbatimStringTok{ }\SpecialCharTok{\{}\NormalTok{sample\_rule}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}

    \CommentTok{\# which indices get written out}
\NormalTok{    idx }\OperatorTok{=} \BuiltInTok{list}\NormalTok{(}\BuiltInTok{range}\NormalTok{(}\DecValTok{1}\NormalTok{, n }\OperatorTok{+} \DecValTok{1}\NormalTok{)) }\ControlFlowTok{if}\NormalTok{ n }\OperatorTok{\textless{}=}\NormalTok{ max\_terms }\ControlFlowTok{else}\NormalTok{ [}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{3}\NormalTok{, n }\OperatorTok{{-}} \DecValTok{1}\NormalTok{, n]}
\NormalTok{    pts }\OperatorTok{=}\NormalTok{ [a }\OperatorTok{+}\NormalTok{ (i }\OperatorTok{{-}}\NormalTok{ offset) }\OperatorTok{*}\NormalTok{ delta }\ControlFlowTok{for}\NormalTok{ i }\KeywordTok{in}\NormalTok{ idx]}
\NormalTok{    pts }\OperatorTok{=}\NormalTok{ [sp.nsimplify(p) }\ControlFlowTok{if}\NormalTok{ p.is\_Rational }\ControlFlowTok{else}\NormalTok{ sp.simplify(p) }\ControlFlowTok{for}\NormalTok{ p }\KeywordTok{in}\NormalTok{ pts]}
\NormalTok{    f\_terms }\OperatorTok{=}\NormalTok{ [}\VerbatimStringTok{rf\textquotesingle{}f}\CharTok{\textbackslash{}!}\ErrorTok{\textbackslash{}}\VerbatimStringTok{left}\KeywordTok{(}\SpecialCharTok{\{}\NormalTok{sp}\SpecialCharTok{.}\NormalTok{latex(p)}\SpecialCharTok{\}}\CharTok{\textbackslash{}r}\VerbatimStringTok{ight}\KeywordTok{)}\VerbatimStringTok{\textquotesingle{}} \ControlFlowTok{for}\NormalTok{ p }\KeywordTok{in}\NormalTok{ pts]}
\NormalTok{    vals }\OperatorTok{=}\NormalTok{ [sp.nsimplify(sp.simplify(f.subs(x, p))) }\ControlFlowTok{for}\NormalTok{ p }\KeywordTok{in}\NormalTok{ pts]}
    \ControlFlowTok{if} \BuiltInTok{all}\NormalTok{(v.is\_Rational }\KeywordTok{and} \BuiltInTok{len}\NormalTok{(sp.latex(v)) }\OperatorTok{\textless{}=} \DecValTok{14} \ControlFlowTok{for}\NormalTok{ v }\KeywordTok{in}\NormalTok{ vals):}
\NormalTok{        val\_terms }\OperatorTok{=}\NormalTok{ [sp.latex(v) }\ControlFlowTok{for}\NormalTok{ v }\KeywordTok{in}\NormalTok{ vals]}
    \ControlFlowTok{else}\NormalTok{:}
\NormalTok{        val\_terms }\OperatorTok{=}\NormalTok{ [\_fmt(sp.N(v)) }\ControlFlowTok{for}\NormalTok{ v }\KeywordTok{in}\NormalTok{ vals]}

\NormalTok{    general }\OperatorTok{=} \VerbatimStringTok{rf\textquotesingle{}RS}\KeywordTok{(}\VerbatimStringTok{P, S}\KeywordTok{)}\VerbatimStringTok{ = }\DecValTok{\textbackslash{}s}\VerbatimStringTok{um\_}\CharTok{\{\{}\VerbatimStringTok{i=1}\CharTok{\}\}}\DecValTok{\^{}}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\CharTok{\}\}}\VerbatimStringTok{ f}\KeywordTok{(}\VerbatimStringTok{x\_i}\DecValTok{\^{}}\OperatorTok{*}\KeywordTok{)}\CharTok{\textbackslash{},}\DecValTok{\textbackslash{}D}\VerbatimStringTok{elta\textquotesingle{}}
\NormalTok{    display(Math(general }\OperatorTok{+} \VerbatimStringTok{rf\textquotesingle{} = }\DecValTok{\textbackslash{}D}\VerbatimStringTok{elta }\ErrorTok{\textbackslash{}}\VerbatimStringTok{left}\PreprocessorTok{[ f(x\_1\^{}*) + f(x\_2\^{}*) + }\ErrorTok{\textbackslash{}}\PreprocessorTok{cdots + f(x\_}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{n}\SpecialCharTok{\}}\CharTok{\}\}}\PreprocessorTok{\^{}*) }\CharTok{\textbackslash{}r}\PreprocessorTok{ight]}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}
\NormalTok{    display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\ErrorTok{\textbackslash{}}\VerbatimStringTok{phantom}\CharTok{\{\{}\VerbatimStringTok{RS}\KeywordTok{(}\VerbatimStringTok{P, S}\KeywordTok{)}\CharTok{\}\}}\VerbatimStringTok{ = }\SpecialCharTok{\{}\NormalTok{D}\SpecialCharTok{\}}\VerbatimStringTok{ }\ErrorTok{\textbackslash{}}\VerbatimStringTok{left}\PreprocessorTok{[ }\SpecialCharTok{\{}\NormalTok{\_join\_terms(f\_terms, n, max\_terms)}\SpecialCharTok{\}}\PreprocessorTok{ }\CharTok{\textbackslash{}r}\PreprocessorTok{ight]}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}
\NormalTok{    display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\ErrorTok{\textbackslash{}}\VerbatimStringTok{phantom}\CharTok{\{\{}\VerbatimStringTok{RS}\KeywordTok{(}\VerbatimStringTok{P, S}\KeywordTok{)}\CharTok{\}\}}\VerbatimStringTok{ = }\SpecialCharTok{\{}\NormalTok{D}\SpecialCharTok{\}}\VerbatimStringTok{ }\ErrorTok{\textbackslash{}}\VerbatimStringTok{left}\PreprocessorTok{[ }\SpecialCharTok{\{}\NormalTok{\_join\_terms(val\_terms, n, max\_terms)}\SpecialCharTok{\}}\PreprocessorTok{ }\CharTok{\textbackslash{}r}\PreprocessorTok{ight]}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}

    \CommentTok{\# exact value when it is a short rational number, else a decimal}
\NormalTok{    exact }\OperatorTok{=} \VariableTok{None}
    \ControlFlowTok{if}\NormalTok{ n }\OperatorTok{\textless{}=} \DecValTok{200}\NormalTok{:}
\NormalTok{        total }\OperatorTok{=}\NormalTok{ sp.nsimplify(}\BuiltInTok{sum}\NormalTok{(f.subs(x, a }\OperatorTok{+}\NormalTok{ (i }\OperatorTok{{-}}\NormalTok{ offset) }\OperatorTok{*}\NormalTok{ delta) }\ControlFlowTok{for}\NormalTok{ i }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(}\DecValTok{1}\NormalTok{, n }\OperatorTok{+} \DecValTok{1}\NormalTok{)) }\OperatorTok{*}\NormalTok{ delta)}
        \ControlFlowTok{if}\NormalTok{ total.is\_Rational }\KeywordTok{and} \BuiltInTok{len}\NormalTok{(sp.latex(total)) }\OperatorTok{\textless{}=} \DecValTok{20}\NormalTok{:}
\NormalTok{            exact }\OperatorTok{=}\NormalTok{ total}
    \ControlFlowTok{if}\NormalTok{ exact }\KeywordTok{is} \KeywordTok{not} \VariableTok{None} \KeywordTok{and}\NormalTok{ exact.q }\OperatorTok{!=} \DecValTok{1}\NormalTok{:}
\NormalTok{        result }\OperatorTok{=} \VerbatimStringTok{rf\textquotesingle{}}\SpecialCharTok{\{}\NormalTok{sp}\SpecialCharTok{.}\NormalTok{latex(exact)}\SpecialCharTok{\}}\VerbatimStringTok{ }\CharTok{\textbackslash{}a}\VerbatimStringTok{pprox }\SpecialCharTok{\{}\NormalTok{\_fmt(value, }\DecValTok{10}\NormalTok{)}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}}
    \ControlFlowTok{else}\NormalTok{:}
\NormalTok{        result }\OperatorTok{=}\NormalTok{ \_fmt(value, }\DecValTok{10}\NormalTok{)}
\NormalTok{    display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\ErrorTok{\textbackslash{}}\VerbatimStringTok{phantom}\CharTok{\{\{}\VerbatimStringTok{RS}\KeywordTok{(}\VerbatimStringTok{P, S}\KeywordTok{)}\CharTok{\}\}}\VerbatimStringTok{ = }\DecValTok{\textbackslash{}m}\VerbatimStringTok{athbf}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{result}\SpecialCharTok{\}}\CharTok{\}\}}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}

    \ControlFlowTok{if}\NormalTok{ show\_integral:}
\NormalTok{        I }\OperatorTok{=}\NormalTok{ sp.integrate(f, (x, a, b))}
\NormalTok{        I\_num }\OperatorTok{=} \BuiltInTok{float}\NormalTok{(sp.N(I))}
        \ControlFlowTok{if} \BuiltInTok{isinstance}\NormalTok{(I, sp.Integral):   }\CommentTok{\# no closed form found}
\NormalTok{            display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\ErrorTok{\textbackslash{}}\VerbatimStringTok{int\_}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{A}\SpecialCharTok{\}}\CharTok{\}\}}\DecValTok{\^{}}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{B}\SpecialCharTok{\}}\CharTok{\}\}}\VerbatimStringTok{ }\SpecialCharTok{\{}\NormalTok{fx}\SpecialCharTok{\}}\CharTok{\textbackslash{},}\VerbatimStringTok{ d}\SpecialCharTok{\{}\NormalTok{x}\SpecialCharTok{\}}\VerbatimStringTok{ }\CharTok{\textbackslash{}a}\VerbatimStringTok{pprox }\SpecialCharTok{\{}\NormalTok{\_fmt(I\_num, }\DecValTok{10}\NormalTok{)}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}
        \ControlFlowTok{else}\NormalTok{:}
\NormalTok{            approx }\OperatorTok{=} \StringTok{\textquotesingle{}\textquotesingle{}} \ControlFlowTok{if}\NormalTok{ I.is\_Rational }\ControlFlowTok{else} \VerbatimStringTok{rf\textquotesingle{} }\CharTok{\textbackslash{}a}\VerbatimStringTok{pprox }\SpecialCharTok{\{}\NormalTok{\_fmt(I\_num, }\DecValTok{10}\NormalTok{)}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}}
\NormalTok{            display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\ErrorTok{\textbackslash{}}\VerbatimStringTok{int\_}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{A}\SpecialCharTok{\}}\CharTok{\}\}}\DecValTok{\^{}}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{B}\SpecialCharTok{\}}\CharTok{\}\}}\VerbatimStringTok{ }\SpecialCharTok{\{}\NormalTok{fx}\SpecialCharTok{\}}\CharTok{\textbackslash{},}\VerbatimStringTok{ d}\SpecialCharTok{\{}\NormalTok{x}\SpecialCharTok{\}}\VerbatimStringTok{ = }\SpecialCharTok{\{}\NormalTok{sp}\SpecialCharTok{.}\NormalTok{latex(I)}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}} \OperatorTok{+}\NormalTok{ approx))}
\NormalTok{        err }\OperatorTok{=}\NormalTok{ value }\OperatorTok{{-}}\NormalTok{ I\_num}
\NormalTok{        err\_tex }\OperatorTok{=} \StringTok{\textquotesingle{}0\textquotesingle{}} \ControlFlowTok{if} \BuiltInTok{abs}\NormalTok{(err) }\OperatorTok{\textless{}} \FloatTok{1e{-}12} \ControlFlowTok{else}\NormalTok{ \_fmt(err, }\DecValTok{4}\NormalTok{)}
\NormalTok{        display(Math(}\VerbatimStringTok{rf\textquotesingle{}}\CharTok{\textbackslash{}t}\VerbatimStringTok{ext}\CharTok{\{\{}\VerbatimStringTok{error: }\CharTok{\}\}}\VerbatimStringTok{ RS}\KeywordTok{(}\VerbatimStringTok{P, S}\KeywordTok{)}\VerbatimStringTok{ {-} }\ErrorTok{\textbackslash{}}\VerbatimStringTok{int\_}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{A}\SpecialCharTok{\}}\CharTok{\}\}}\DecValTok{\^{}}\CharTok{\{\{}\SpecialCharTok{\{}\NormalTok{B}\SpecialCharTok{\}}\CharTok{\}\}}\VerbatimStringTok{ f}\CharTok{\textbackslash{},}\VerbatimStringTok{d}\SpecialCharTok{\{}\NormalTok{x}\SpecialCharTok{\}}\VerbatimStringTok{ }\CharTok{\textbackslash{}a}\VerbatimStringTok{pprox }\SpecialCharTok{\{}\NormalTok{err\_tex}\SpecialCharTok{\}}\VerbatimStringTok{\textquotesingle{}}\NormalTok{))}

    \ControlFlowTok{return}\NormalTok{ value}
\end{Highlighting}
\end{Shaded}

\section{Recap: the setup from
lecture}\label{recap-the-setup-from-lecture}
...
\tightlist
\item
  A partition \(P\) of \([a,b]\) is a choice of points
  \(a = x_0 < x_1 < \cdots < x_n = b\).
\item
  \(\Delta_i = x_i - x_{i-1}\) is the length of the \(i\)-th segment
  \([x_{i-1}, x_i]\).
\item
  Sample points \(S = \{x_1^*, \ldots, x_n^*\}\) with
  \(x_i^* \in [x_{i-1}, x_i]\) determine the heights \(f(x_i^*)\) of the
  rectangles.
\item
  The Riemann sum is
...
In this notebook we always split \([a,b]\) into \(n\) segments of equal
length, so \(\Delta_i = \Delta = \frac{b-a}{n}\) and
\(x_i = a + i\Delta\). There are three standard ways to pick the sample
points:
...
\bottomrule\noalign{}
\endlastfoot
\texttt{\textquotesingle{}left\textquotesingle{}} & \(x_i^* = x_{i-1}\),
the left endpoint \\
\texttt{\textquotesingle{}right\textquotesingle{}} & \(x_i^* = x_i\),
the right endpoint \\
\texttt{\textquotesingle{}midpoint\textquotesingle{}} &
\(x_i^* = \frac{x_{i-1}+x_i}{2}\), the midpoint \\
\end{longtable}
}

Run the next cell to define the function \texttt{riemann\_sum}. You do
not need to read its code to use it.

\subsection{\texorpdfstring{How to call
\texttt{riemann\_sum}}{How to call riemann\_sum}}\label{how-to-call-riemann_sum}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(f, a, b, n, method)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \texttt{f} is an expression in \texttt{x}, for example \texttt{x**2},
  \texttt{sp.sin(x)}, \texttt{sp.exp(-x**2)}, \texttt{1/x},
  \texttt{sp.sqrt(1\ -\ x**2)} (Recall: exponentiation in Python is
  \texttt{**}. The prefix \texttt{sp.} instructs python to use a library
  for symbolic computation called \texttt{sympy}.)
\item
  \texttt{a}, \texttt{b} are the bounds with \texttt{a\ \textless{}\ b};
  exact values like \texttt{sp.pi} or \texttt{sp.Rational(1,\ 3)} are
  fine
\item
  \texttt{n} is the number of subintervals
\item
  \texttt{method} is \texttt{\textquotesingle{}left\textquotesingle{}},
  \texttt{\textquotesingle{}right\textquotesingle{}}, or
  \texttt{\textquotesingle{}midpoint\textquotesingle{}}
\end{itemize}

Optional arguments: - \texttt{show\_integral=True} also displays the
exact integral and the error of the Riemann sum - \texttt{show=False}
skips the picture and just returns the number (useful in loops) -
\texttt{max\_terms=6} controls how many terms are written out before the
sum is abbreviated with \(+ \cdots +\)

The function returns the value of the sum. End the line with a semicolon
\texttt{;} so Jupyter does not print that number a second time.

\section{Part 1: Start}\label{part-1-start}

We start with \(f(x) = x\) on \([1,2]\) and \(n = 5\) subintervals.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(x, }\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{5}\NormalTok{, }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-3-output-1.png}}

$\displaystyle f(x) = x, \qquad [a, b] = [1, 2], \qquad n = 5$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{2 - 1}{5} = \frac{1}{5}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 5)$

$\displaystyle \text{left sample points: } x_i^* = x_{i-1} = a + (i-1)\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{5} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{5}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{5} \left[ f\!\left(1\right) + f\!\left(\frac{6}{5}\right) + f\!\left(\frac{7}{5}\right) + f\!\left(\frac{8}{5}\right) + f\!\left(\frac{9}{5}\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{5} \left[ 1 + \frac{6}{5} + \frac{7}{5} + \frac{8}{5} + \frac{9}{5} \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{\frac{7}{5} \approx 1.4}$

In class, we computed the actual value of the integral
\(\int_a^b x dx\). How does this value compare? Increase \texttt{n} in
the cell above and see what happens.

\section{Part 2: Left, right, and midpoint
sums}\label{part-2-left-right-and-midpoint-sums}

Now we look at the function \(f(x) = x^2\) on \([0,1]\) and \(n = 4\)
subintervals.

\textbf{PREDICT} (on paper, before running anything): - Will the left
sum overestimate or underestimate the area under \(x^2\)? What about the
right sum? - Compute the left sum by hand. You should need nothing more
than adding four fractions.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(x}\OperatorTok{**}\DecValTok{2}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{4}\NormalTok{, }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-4-output-1.png}}

$\displaystyle f(x) = x^{2}, \qquad [a, b] = [0, 1], \qquad n = 4$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{1 - 0}{4} = \frac{1}{4}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 4)$

$\displaystyle \text{left sample points: } x_i^* = x_{i-1} = a + (i-1)\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{4} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{4}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{4} \left[ f\!\left(0\right) + f\!\left(\frac{1}{4}\right) + f\!\left(\frac{1}{2}\right) + f\!\left(\frac{3}{4}\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{4} \left[ 0 + \frac{1}{16} + \frac{1}{4} + \frac{9}{16} \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{\frac{7}{32} \approx 0.21875}$

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(x}\OperatorTok{**}\DecValTok{2}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{4}\NormalTok{, }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-5-output-1.png}}

$\displaystyle f(x) = x^{2}, \qquad [a, b] = [0, 1], \qquad n = 4$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{1 - 0}{4} = \frac{1}{4}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 4)$

$\displaystyle \text{right sample points: } x_i^* = x_i = a + i\,\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{4} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{4}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{4} \left[ f\!\left(\frac{1}{4}\right) + f\!\left(\frac{1}{2}\right) + f\!\left(\frac{3}{4}\right) + f\!\left(1\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{4} \left[ \frac{1}{16} + \frac{1}{4} + \frac{9}{16} + 1 \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{\frac{15}{32} \approx 0.46875}$

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(x}\OperatorTok{**}\DecValTok{2}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{4}\NormalTok{, }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{, show\_integral}\OperatorTok{=}\VariableTok{True}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-6-output-1.png}}

$\displaystyle f(x) = x^{2}, \qquad [a, b] = [0, 1], \qquad n = 4$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{1 - 0}{4} = \frac{1}{4}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 4)$

$\displaystyle \text{midpoint sample points: } x_i^* = \frac{x_{i-1} + x_i}{2} = a + \left(i - \tfrac{1}{2}\right)\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{4} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{4}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{4} \left[ f\!\left(\frac{1}{8}\right) + f\!\left(\frac{3}{8}\right) + f\!\left(\frac{5}{8}\right) + f\!\left(\frac{7}{8}\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{4} \left[ \frac{1}{64} + \frac{9}{64} + \frac{25}{64} + \frac{49}{64} \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{\frac{21}{64} \approx 0.328125}$

$\displaystyle \int_{0}^{1} x^{2}\, dx = \frac{1}{3}$

$\displaystyle \text{error: } RS(P, S) - \int_{0}^{1} f\,dx \approx -0.005208$

\textbf{VERIFY:} The left and right sums share almost all of their
terms. Show that for any \(f\) and equally spaced partition,
\[RS_{\text{right}} - RS_{\text{left}} = \Delta\,\bigl(f(b) - f(a)\bigr),\]
and check this against the two numbers you just computed.

\textbf{REFLECT:} Your prediction about over- and underestimates
depended on a property of \(x^2\) on \([0,1]\). Which property? State a
general claim about left and right sums for functions with that
property.

\section{Part 3: Refining the
partition}\label{part-3-refining-the-partition}

In lecture, the approximation got better by refining the partition. Let
us watch that happen. Once \(n\) is larger than 6, the written-out sum
is abbreviated with \(+ \cdots +\).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(x}\OperatorTok{**}\DecValTok{2}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{16}\NormalTok{, }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, show\_integral}\OperatorTok{=}\VariableTok{True}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-7-output-1.png}}

$\displaystyle f(x) = x^{2}, \qquad [a, b] = [0, 1], \qquad n = 16$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{1 - 0}{16} = \frac{1}{16}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 16)$

$\displaystyle \text{left sample points: } x_i^* = x_{i-1} = a + (i-1)\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{16} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{16}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{16} \left[ f\!\left(0\right) + f\!\left(\frac{1}{16}\right) + f\!\left(\frac{1}{8}\right) + \cdots + f\!\left(\frac{7}{8}\right) + f\!\left(\frac{15}{16}\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{16} \left[ 0 + 0.00390625 + 0.015625 + \cdots + 0.765625 + 0.878906 \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{\frac{155}{512} \approx 0.302734375}$

$\displaystyle \int_{0}^{1} x^{2}\, dx = \frac{1}{3}$

$\displaystyle \text{error: } RS(P, S) - \int_{0}^{1} f\,dx \approx -0.0306$

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(x}\OperatorTok{**}\DecValTok{2}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{200}\NormalTok{, }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, show\_integral}\OperatorTok{=}\VariableTok{True}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-8-output-1.png}}

$\displaystyle f(x) = x^{2}, \qquad [a, b] = [0, 1], \qquad n = 200$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{1 - 0}{200} = \frac{1}{200}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 200)$

$\displaystyle \text{left sample points: } x_i^* = x_{i-1} = a + (i-1)\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{200} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{200}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{200} \left[ f\!\left(0\right) + f\!\left(\frac{1}{200}\right) + f\!\left(\frac{1}{100}\right) + \cdots + f\!\left(\frac{99}{100}\right) + f\!\left(\frac{199}{200}\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{200} \left[ 0 + 2.5 \cdot 10^{-5} + 0.0001 + \cdots + 0.9801 + 0.990025 \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{\frac{26467}{80000} \approx 0.3308375}$

$\displaystyle \int_{0}^{1} x^{2}\, dx = \frac{1}{3}$

$\displaystyle \text{error: } RS(P, S) - \int_{0}^{1} f\,dx \approx -0.002496$

\section{Part 4: Back to the circle}\label{part-4-back-to-the-circle}

We started the Riemann integral lecture by approximating the area
\(\pi\) of the unit circle with regular \(n\)-gons. The quarter of the
unit disk in the first quadrant is the region under
\(f(x) = \sqrt{1 - x^2}\) on \([0,1]\), so
\(4\int_0^1 \sqrt{1-x^2}\,dx = \pi\).

\textbf{PREDICT:} Is the left sum of \(\sqrt{1-x^2}\) on \([0,1]\) an
overestimate or an underestimate? Why?

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(sp.sqrt(}\DecValTok{1} \OperatorTok{{-}}\NormalTok{ x}\OperatorTok{**}\DecValTok{2}\NormalTok{), }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{5}\NormalTok{, }\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, show\_integral}\OperatorTok{=}\VariableTok{True}\NormalTok{)}\OperatorTok{;}

\CommentTok{\# }\AlertTok{TODO}\CommentTok{: find the smallest n for which 4 * (left sum) is within 10**({-}3) of pi.}
\CommentTok{\#   Use riemann\_sum(..., show=False) inside a loop.}
\CommentTok{\# }\AlertTok{TODO}\CommentTok{: do the same for the midpoint sum. How much smaller is the n you need?}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-9-output-1.png}}

$\displaystyle f(x) = \sqrt{1 - x^{2}}, \qquad [a, b] = [0, 1], \qquad n = 5$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{1 - 0}{5} = \frac{1}{5}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 5)$

$\displaystyle \text{left sample points: } x_i^* = x_{i-1} = a + (i-1)\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{5} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{5}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{5} \left[ f\!\left(0\right) + f\!\left(\frac{1}{5}\right) + f\!\left(\frac{2}{5}\right) + f\!\left(\frac{3}{5}\right) + f\!\left(\frac{4}{5}\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{5} \left[ 1 + 0.979796 + 0.916515 + 0.8 + 0.6 \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{0.8592622072}$

$\displaystyle \int_{0}^{1} \sqrt{1 - x^{2}}\, dx = \frac{\pi}{4} \approx 0.7853981634$

$\displaystyle \text{error: } RS(P, S) - \int_{0}^{1} f\,dx \approx 0.07386$

\section{Part 5: Experiment on your
own}\label{part-5-experiment-on-your-own}

Change the function, bounds, \(n\), and method in the cell below and run
it again. If \texttt{ipywidgets} is installed, the cell after it gives
you sliders instead.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{f }\OperatorTok{=}\NormalTok{ sp.exp(}\OperatorTok{{-}}\NormalTok{x}\OperatorTok{**}\DecValTok{2}\NormalTok{)}
\NormalTok{riemann\_sum(f, }\OperatorTok{{-}}\DecValTok{2}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{10}\NormalTok{, }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{, show\_integral}\OperatorTok{=}\VariableTok{True}\NormalTok{)}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-10-output-1.png}}

$\displaystyle f(x) = e^{- x^{2}}, \qquad [a, b] = [-2, 2], \qquad n = 10$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{2 - \left(-2\right)}{10} = \frac{2}{5}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 10)$

$\displaystyle \text{midpoint sample points: } x_i^* = \frac{x_{i-1} + x_i}{2} = a + \left(i - \tfrac{1}{2}\right)\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{10} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{10}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{2}{5} \left[ f\!\left(- \frac{9}{5}\right) + f\!\left(- \frac{7}{5}\right) + f\!\left(-1\right) + \cdots + f\!\left(\frac{7}{5}\right) + f\!\left(\frac{9}{5}\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{2}{5} \left[ 0.0391639 + 0.140858 + 0.367879 + \cdots + 0.140858 + 0.0391639 \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{1.765094018}$

$\displaystyle \int_{-2}^{2} e^{- x^{2}}\, dx = \sqrt{\pi} \operatorname{erf}{\left(2 \right)} \approx 1.764162782$

$\displaystyle \text{error: } RS(P, S) - \int_{-2}^{2} f\,dx \approx 0.0009312$

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Interactive version: move the slider or change the method}
\ControlFlowTok{try}\NormalTok{:}
    \ImportTok{from}\NormalTok{ ipywidgets }\ImportTok{import}\NormalTok{ interact, IntSlider, Dropdown}

    \KeywordTok{def}\NormalTok{ explore(n}\OperatorTok{=}\DecValTok{4}\NormalTok{, method}\OperatorTok{=}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{):}
\NormalTok{        riemann\_sum(sp.sin(x) }\OperatorTok{+} \FloatTok{1.5}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{2}\OperatorTok{*}\NormalTok{sp.pi, n, method, show\_integral}\OperatorTok{=}\VariableTok{True}\NormalTok{)}

\NormalTok{    interact(explore,}
\NormalTok{             n}\OperatorTok{=}\NormalTok{IntSlider(}\BuiltInTok{min}\OperatorTok{=}\DecValTok{1}\NormalTok{, }\BuiltInTok{max}\OperatorTok{=}\DecValTok{100}\NormalTok{, step}\OperatorTok{=}\DecValTok{1}\NormalTok{, value}\OperatorTok{=}\DecValTok{4}\NormalTok{),}
\NormalTok{             method}\OperatorTok{=}\NormalTok{Dropdown(options}\OperatorTok{=}\NormalTok{[}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}midpoint\textquotesingle{}}\NormalTok{], value}\OperatorTok{=}\StringTok{\textquotesingle{}left\textquotesingle{}}\NormalTok{))}\OperatorTok{;}
\ControlFlowTok{except} \PreprocessorTok{ImportError}\NormalTok{:}
    \BuiltInTok{print}\NormalTok{(}\StringTok{\textquotesingle{}ipywidgets is not installed; use the cell above instead.\textquotesingle{}}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\section{Part 6: when Riemann sums
fail}\label{part-6-when-riemann-sums-fail}

Riemann sums are defined for bounded functions. Look at what happens for
\(f(x) = 1/x\) on \([0,1]\) using right sums (the left sum would need
\(f(0)\), which does not exist).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{riemann\_sum(}\DecValTok{1}\OperatorTok{/}\NormalTok{x, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{10}\NormalTok{, }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{)}\OperatorTok{;}
...
\ControlFlowTok{for}\NormalTok{ n }\KeywordTok{in}\NormalTok{ [}\DecValTok{10}\NormalTok{, }\DecValTok{100}\NormalTok{, }\DecValTok{1000}\NormalTok{, }\DecValTok{10000}\NormalTok{]:}
    \BuiltInTok{print}\NormalTok{(n, riemann\_sum(}\DecValTok{1}\OperatorTok{/}\NormalTok{x, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, n, }\StringTok{\textquotesingle{}right\textquotesingle{}}\NormalTok{, show}\OperatorTok{=}\VariableTok{False}\NormalTok{))}

\CommentTok{\# }\AlertTok{TODO}\CommentTok{: what happens to the sums as n grows? Compare with 1/sp.sqrt(x) on [0, 1].}
\CommentTok{\#   The first rectangle is always very tall in both cases, }
\CommentTok{\#    so why do the two behave so differently?}
\end{Highlighting}
\end{Shaded}

\pandocbounded{\includegraphics[keepaspectratio]{notebooks/Week05_Riemann_Sums_files/figure-pdf/cell-12-output-1.png}}

$\displaystyle f(x) = \frac{1}{x}, \qquad [a, b] = [0, 1], \qquad n = 10$

$\displaystyle \Delta = \frac{b-a}{n} = \frac{1 - 0}{10} = \frac{1}{10}, \qquad x_i = a + i\,\Delta \quad (i = 0, 1, \ldots, 10)$

$\displaystyle \text{right sample points: } x_i^* = x_i = a + i\,\Delta$

$\displaystyle RS(P, S) = \sum_{i=1}^{10} f(x_i^*)\,\Delta = \Delta \left[ f(x_1^*) + f(x_2^*) + \cdots + f(x_{10}^*) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{10} \left[ f\!\left(\frac{1}{10}\right) + f\!\left(\frac{1}{5}\right) + f\!\left(\frac{3}{10}\right) + \cdots + f\!\left(\frac{9}{10}\right) + f\!\left(1\right) \right]$

$\displaystyle \phantom{RS(P, S)} = \frac{1}{10} \left[ 10 + 5 + \frac{10}{3} + \cdots + \frac{10}{9} + 1 \right]$

$\displaystyle \phantom{RS(P, S)} = \mathbf{\frac{7381}{2520} \approx 2.928968254}$
